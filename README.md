...
1. Генеративный дизайн новых трехфазных структур
...
<hr>
<i>Дополнено 23.02.2026 - проработка прикладного применения</i>
# 4. Построение графов по двумерным изображениями трехфазной структуры и обучение модели (по аналогии с п.3.)

Учитывая, что получение изображений на FIB-SEM (двулучевой сканирующий электронно-ионный микроскоп) дорогостоящая и длительная во времени операция, целесообразно проработать извлечение графов и обучение graph-CVAE модели по исходным 2D изображениям.

# 4.1. Построение графов
Алгоритм извлечения графов взят из описанному в п.3.2. для того, чтобы не менять архитектуру и пайплайн обучения все несуществующие в двумерном изображении координаты `z` при построения графа принимаются равными `0`. Т.е. коодинаты вершин графа имеют вид `[0, y, x]`

Образцы взяты из того же датасета "FIB-tomography data of Ni-YSZ anodes for Solid Oxide Fuel Cells (SOFC): Comparison of pristine and degraded materials (before/after redox cycling)" (Holzer, L., Pecho, O., Hocker, T., Iwanschitz, B., & Mai, A. (2020). FIB-tomography data of Ni-YSZ anodes for Solid Oxide Fuel Cells (SOFC): Comparison of pristine and degraded materials (before/after redox cycling) (Версия 1) [Data set]. Zenodo. https://doi.org/10.5281/zenodo.4056538)

В качестве образцов взят каждый 64й срез (изображение) из датасета.

<img width="600" src="https://github.com/user-attachments/assets/e9d94d41-35ef-4a70-8617-fa46d0cffb7a" />

Количество образцов на каждый класс - 144 снимка.

Общее количество образцов обучающего датасета (по 6ти классам) - 864 снимка.

## Визуализация графов по изображениям трехфазной структуры

<img height="800" src="https://github.com/user-attachments/assets/c3963c29-21ce-41c4-b982-7a4ff2cc778c" />


# 4.2. Обучение модели Graph Conditional VAE по графам 2D снимков

<img height="800" alt="Обучение модели Graph Conditional VAE по графам 2D снимков" src="https://github.com/user-attachments/assets/02ff325c-051c-49d0-b786-d2f9e287a2ee" />

## Визуализация латентного пространства

<img height="600" alt="Латентное пространство 2D_CVAE" src="https://github.com/user-attachments/assets/0e36a864-2f71-4216-9271-e32f10b8fa6f" />

Однотипные классы сгруппированы очень плотно, классы "pristine" и "degraded" распределены очень четко и строго. 
Визуальная интерпретация латентного пространства очень хорошая.


## Результаты: 
Conditional VAE использующий на вход 2D графы также хорошо справился с распределением латентного пространства, все соответствующие точки сгруппированы, образцы до и после деградации находятся на существенном удалении друг от друга. Целесообразно применение данной модели для дальнейшей проработки

## Визуализация реконструированных трехфазных структур

<img height="800" alt="Реконструктированный образец 1" src="https://github.com/user-attachments/assets/8edcc727-d8d8-4c31-a8c6-07d39bbdc5d4" />

<img height="800" alt="Реконструктированный образец 2" src="https://github.com/user-attachments/assets/8a74d53c-b40d-45ca-a89d-daa5eaede93b" />

Лишние вершины графа формируются из-за паддинга в `loss` и отсутствия маскировки/штрафов за нулевые/изолированные узлы в decoder, что приводит к "галлюцинациям" мелких компонент, не присутствующих в оригинале.
Возможно тербуется добавить маску в `loss` для игнора паддинга.
